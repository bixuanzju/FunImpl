...
\item If given $\Gamma \vdash \sigma:\star$, a polymorphic fixed-point
  constructor $\FP:(\Pi \alpha:\star.(\alpha \to \alpha) \to \alpha)$
...
\item Using $\FP$, we can build recursive functions. For example,
  given a ``hungry'' type $H=\HGType$, the ``hungry'' function $f$
  where
\[f=\lambda \alpha:\star.\FP\,(\alpha \to H)\,(\lambda f:\alpha \to H.\lambda
x:\alpha.\fold{H}{f})\] can take arbitrary number of arguments.
\end{itemize}
