...
\begin{figure}[H]
  ...
  \begin{syntax}
    ...
    decl &::= & \hlmath{\data\,D = \overline{K \ \overline{\tau}}} & \ptext{Datatype} \\ \\
    \textbf{Terms} \\
    u &::= & \hlmath{x \mid K} & \ptext{Variables and data constructors} \\
    e,\tau &::= & \hlmath{u} & \ptext{Term atoms} \\
    & \mid & \star & \ptext{Star} \\
    ...
    & \mid & \fold{\miu{x}{\tau}}{e} & \ptext{Roll} \\
    & \mid & \unfold{e} & \ptext{Unroll} \\
    & \mid & \bet{e} & \ptext{Type reduction} \\
    & \mid & \hlmath{\case\,e\,\of\,\overline{p \Rightarrow e}} & \ptext{Case analysis} \\
    p &::= & \hlmath{K\,\overline{x : \tau}} & \ptext{Pattern} \\ \\
    \textbf{Environments} \\
...
\begin{figure}[ht!]
  ...
  \begin{tabular}{lcl}
    ...
    (Fold) & \ruleII{\ctx{e:(\tau[x:=\miu{x}{\tau}]) \rightsquigarrow E}}{\ctx{\miu{x}{\tau}:s}}%
             {\ctx{(\fold{\miu{x}{\tau}}{e}):\miu{x}{\tau} \rightsquigarrow \fold{\miu{x}{\tau}}{E}}} \\

    (Unfold) & \ruleII{\ctx{e:\miu{x}{\tau} \rightsquigarrow E}}{\ctx{\tau[x:=\miu{x}{\tau}]:s}}%
               {\ctx{(\unfold{e}):\tau[x:=\miu{x}{\tau}] \rightsquigarrow \unfold{E}}} \\

    (Beta) & \ruleIII{\ctx{e:\tau_{1} \rightsquigarrow E}}{\ctx{\tau_{2}:s}}{\tau_{1} \tolong \tau_{2}}{\ctx{(\bet{e}):\tau_2 \rightsquigarrow \bet{E}}} \\
