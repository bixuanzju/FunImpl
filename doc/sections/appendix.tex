...
\begin{proof}
	Immediate from Lemma \ref{lem:unired}, by induction on the number of reduction steps.
\end{proof}
...
\begin{proof}
	...
	\begin{description}
		...
		\item[Case $\ottnt{e}  \ottsym{=}  \ottmv{x}$:] Trivial by rule \ruleref{TS\_Var} and $\tau$ is the unique type of $\ottmv{x}$ if $\ottmv{x}  \ottsym{:}  \tau \, \in \, \Gamma$.
