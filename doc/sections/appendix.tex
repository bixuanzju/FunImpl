...
Given $ \Sigma  \labeledjudge{s}  \ottnt{E}  :  T $, if $ \labeledjudge{ct}  \Sigma   \rightsquigarrow   \Gamma $, $ \Sigma  \labeledjudge{s}  \ottnt{E}  :  T   \rightsquigarrow   \ottnt{e} $ and $ \Sigma  \labeledjudge{s}  T  :  \star   \rightsquigarrow   \tau $, then $\Gamma  \vdash  \ottnt{e}  \ottsym{:}  \tau$.
\end{lem}

\begin{proof}
    By induction on the derivation of $ \Sigma  \labeledjudge{s}  \ottnt{E}  :  T $:
    \begin{description}
        \item[Case $\ottnt{E}  \ottsym{=}  \star$:]
    \end{description}
\end{proof}

\begin{lem}[Type-safety of datatype translation]
Given $ \Sigma  \labeledjudge{d}  \ottnt{decl}  :  \Sigma' $, if $ \labeledjudge{ct}  \Sigma   \rightsquigarrow   \Gamma $, $ \Sigma  \labeledjudge{d}  \ottnt{decl}  :  \Sigma'   \rightsquigarrow   \ottnt{e} $ and \fixme{$\tau=?$}, then $\Gamma  \vdash  \ottnt{e}  \ottsym{:}  \tau$.
\end{lem}
