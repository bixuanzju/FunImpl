...
have $ \labeledjudge{wf}  \Sigma   \rightsquigarrow   \Gamma $, then $\ottmv{x}:\tau  \in  \Gamma$ where $ \Sigma  \labeledjudge{s}  T  :  \star   \rightsquigarrow   \tau $.
        ...
        \item[Case \ruleref{TR\_Case}:] Suppose 
            ...
            \begin{align*}
                ...
                & \longrightarrow ~ \mathscale[0.7]{\ottsym{(}  \lambda  \,\overline{  \ottnt{u}  \ottsym{:}  \rho  }^{n}\,  \ottsym{.}  \ottsym{(}  \alpha  \ottsym{:}  \star  \ottsym{)}  \rightarrow  \,\overline{  \ottsym{(}  \ottsym{(}  \,\overline{  \ottmv{x}  \ottsym{:}  \tau  \ottsym{[}  \ottmv{D}  \mapsto  \ottmv{X}  \ottsym{]} \,  \ottsym{[}  \ottmv{X}  \mapsto  \ottmv{D}  \ottsym{]} \,  }\,  \ottsym{)}  \rightarrow  \alpha  \ottsym{)}  }\,  \rightarrow  \alpha  \ottsym{)}\,\overline{  u'  }\,^n}\\
                & \longrightarrow_n ~ \ottsym{(}  \alpha  \ottsym{:}  \star  \ottsym{)}  \rightarrow  \,\overline{  \ottsym{(}  \,\overline{  \ottmv{x}  \ottsym{:}  \tau'  }\,  \ottsym{)}  \rightarrow  \alpha  }\,  \rightarrow  \alpha
            \end{align*}
            Then by
            rule \ruleref{T\_CastDown} and the definition of $n$-step cast operator, the
            type of $\mathsf{cast}_{\downarrow}^{n+1} \, \ottnt{e_{{\mathrm{1}}}}$ is \[ \ottsym{(}  \alpha  \ottsym{:}  \star  \ottsym{)}  \rightarrow  \,\overline{  \ottsym{(}  \,\overline{  \ottmv{x}  \ottsym{:}  \tau'  }\,  \ottsym{)}  \rightarrow  \alpha  }\,  \rightarrow  \alpha.\] Note
