...
rules and meta-theory of \ecore. The full specification and proofs 
...
The implementation of type checking is simplified as well. Because
the typing rules of \ecore are syntax-directed without using the
implicit conversion rule, like the rule \ruleref{Tcc\_Conv} of \cc.

\paragraph{Syntactic Sugar}
For the brevity purpose, we use syntactic sugar $[[foldn]]$ and $[[unfoldn]]$
for translation rules in Section \ref{sec:surface} to denote $n$ consecutive cast operators.
Suppose we have $[[e]] : [[t]]$ and sequences of
reduction $[[t1]] [[-->]] [[t2]] [[-->]] \dots [[-->]] [[tn]] [[-->]] [[t]]$
and $[[t]] [[-->]] [[T1]] [[-->]] [[T2]] [[-->]] \dots [[-->]] [[Tn]]$. 
The $n$-step \cast operators can be defined by:
\begin{flalign*}
...
( [[castdown]]}_n [[e]]) \dots ))
\end{flalign*}
Moreover, due to the decidability of one-step reduction (see Section \ref{sec:ecore:meta}), 
the intermediate types  $[[t2]], \dots, [[tn]]$ can be uniquely determined, 
therefore can be left out  from the $n$-step $[[castup]]$ operator:
\[
    [[foldn [t1] e]] [[:=]] [[castup]] [ [[t1]] ]
([[castup]] [ [[t2]] ] (\dots ( [[castup]] [ [[tn]] ] [[e]] ) \dots ).
\]
