...
this paper.  \bruno{Mention where to find the proofs!}
...
\]
\[
\inferrule{[[G |- e2 : int]] \\ [[G |- (\x : star.x) int : star]] \\ [[(\x :
star.x) int --> int]]}{[[G |- (castup[(\x : star.x) int] e2):(\x : star.x)
int]]}
\]
...
The ``Type-in-type'' axiom by rule \ruleref{T\_Ax} brings the benefit 
of supporting \emph{higher-kind polymorphism} in \ecore.
In \cc, $[[Pi x:t1.t2]]$ can be used as a
\emph{dependent product}, if $[[x]]:[[t1]]:[[star]]$ and
$[[t2]]:[[square]]$. However, if the product 
takes a kind parameter like $[[Pi
    x:square.x->x]]$, it is no longer a legal type in \cc. Since $[[square]]$ is
the highest sort that does not have a type, the product cannot type check.
In contrast, \ecore does not have such limitation. Because of
the $[[star]]:[[star]]$ axiom, $[[Pi x:star.x->x]]$ is 
