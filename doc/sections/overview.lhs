...
be simply represented as |mu Nat : * . pi B : * . B -> (Nat -> B) -> B|.

As can be seen, in the function type |B -> (Nat -> B) -> B|, $B$
corresponds to the type of the \emph{Z} constructor, and |X -> B|
