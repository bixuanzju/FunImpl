...
$ \kw{Int} $. What would happen if we try to type check the following
...
introducing two language constructs: $ \kw{cast}_{\downarrow} $ (beta reduction)
and $ \kw{cast}^{\uparrow} $ (beta expansion). The benefit of this approach is
...
$ \kw{cast}_{\downarrow} $, allows a type conversion provided that the resulting
type is a \emph{beta reduction} of the original type of the term. The
use of $ \kw{cast}_{\downarrow} $ is better explained by the following simple
example. Suppose that
\[ g \equiv \lambda  \ottmv{x}  \ottsym{:}  \kw{Int}  \ottsym{.}  \ottmv{x} \]
and term $z$ has type
\[ \ottsym{(}  \lambda  \ottmv{y}  \ottsym{:}  \star  \ottsym{.}  \ottmv{y}  \ottsym{)} \, \kw{Int} \]
$ g\,z $ is an ill-typed application, whereas $ g\,(\kw{cast}_{\downarrow} \, \ottmv{z}) $
is type correct. This is witnessed by
$\ottsym{(}  \lambda  \ottmv{y}  \ottsym{:}  \star  \ottsym{.}  \ottmv{y}  \ottsym{)} \, \kw{Int} \rightarrow_{\beta}  \kw{Int} $, which is a beta
reduction for term $\ottsym{(}  \lambda  \ottmv{y}  \ottsym{:}  \star  \ottsym{.}  \ottmv{y}  \ottsym{)} \, \kw{Int}$. \bruno{explain why this is a
  reduction} \jeremy{done!}

\paragraph{Beta Expansion} The dual operation of $ \kw{cast}_{\downarrow} $ is
$ \kw{cast}^{\uparrow} $, which allows a type conversion provided that the resulting
...
\[ f\,(\kw{cast}^{\uparrow} \, \ottsym{[}  \ottsym{(}  \lambda  \ottmv{y}  \ottsym{:}  \star  \ottsym{.}  \ottmv{y}  \ottsym{)} \, \kw{Int}  \ottsym{]} \,  3) \]
\bruno{how to put a space before $3$?} \jeremy{fixed!} The intuition
is that, $ \kw{cast}^{\uparrow} $ is actually doing a type conversion becuase the
type of $ 3 $ is $  \kw{Int}  $, and $ \ottsym{(}  \lambda  \ottmv{y}  \ottsym{:}  \star  \ottsym{.}  \ottmv{y}  \ottsym{)} \, \kw{Int} $ is the beta
expansion of $ \kw{Int} $ (witnessed by
$\ottsym{(}  \lambda  \ottmv{y}  \ottsym{:}  \star  \ottsym{.}  \ottmv{y}  \ottsym{)} \, \kw{Int} \rightarrow_{\beta}  \kw{Int} $). \bruno{explain why
  this is a beta expansion} \jeremy{done!} Notice that for
$ \kw{cast}^{\uparrow} $ to work, we need to provide the resulting type as
...
application even with the use of $ \kw{cast}^{\uparrow} $ and/or $ \kw{cast}_{\downarrow} $:
one would need to write infinite number of $ \kw{cast}_{\downarrow} $'s to make
the type checker loop forever (e.g.,
$(\lambda  \ottmv{x}  \ottsym{:}  \ottmv{d} \, 3  \ottsym{.}  \ottmv{x})( \kw{cast}_{\downarrow} ( \kw{cast}_{\downarrow}  \dots z))$). But it is
...
isomorphism is witnessed by first $ \kw{cast}^{\uparrow} $, then
$ \kw{cast}_{\downarrow} $. \bruno{Explain that the casts generalize fold and
  unfold!}  \jeremy{done!} At first sight, the cast rules share some
similarities with \fold and \unfold, but $ \kw{cast}^{\uparrow} $ and
$ \kw{cast}_{\downarrow} $ actually generalize \fold and \unfold: they can convert
