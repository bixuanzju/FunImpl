...
\renewcommand{\ottdrulename}[1]{\textsc{\replace{#1}{TR}{T}}}
...
A:star~>t]]$.
        ...
        \item[Case \ruleref{TR\_Case}:] Suppose 
            ...
            \begin{align*}
                [[D]] [[<<uu'>>]]^n~
                &[[-->]]~ \mathscale[0.7]{[[(\ <<u:ro>>n . (aa:star) -> << ((<<x : t[D |-> X][X |-> D]>>) -> aa) >> -> aa) ]][[<<uu'>>]]^n}\\
                &[[-->>]]~ [[(aa:star) -> << (<<x:t'>>) -> aa >> -> aa]]
            \end{align*}
            Then by
            rule \ruleref{T\_CastDown} and the definition of $n$-step cast operator, the
            type of $[[unfoldnp e1]]$ is \[ [[(aa:star) -> << (<<x:t'>>) -> aa >> -> aa]].\] Note
            that by rule \ruleref{T\_Lam}, $[[G |- e2 : (<<x:t'>>) -> T]]$. Therefore, by rule
            \ruleref{T\_App}, we obtain $[[G |- (unfoldnp e1) T <<e2>> : T]]$, which follows
            the result.
    \end{description}
\end{proof}
