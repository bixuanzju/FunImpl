...
cases:
    \begin{description}
        \item[Case \ruleref{T\_Mu}:] $\inferrule{[[G1, x:T, G2 |- e1:t]] \\
...
\begin{proof}
	...
	\begin{description}
		...
		\begin{inparaenum}[(1)]
			...
			\item $[[castup [t] e]]$,
		\end{inparaenum}
		which cannot match any rules of $[[-->]]$. Thus there is no $[[e]]'$ such
that $[[e-->e']]$.
		\item[Case $[[e]]=[[(\x:t.e1) e2]]$:] There is a unique $[[e]]'=[[ e1[x|->e2]
]]$ by rule \ruleref{S\_Beta}.
		\item[Case $[[e]]=[[castdown (castup [t] e)]]$:] There is a unique $[[e]]'=e$
by rule \ruleref{S\_CastDownUp}.
		\item[Case $[[e]]=[[mu x:t.e]]$:] There is a unique $[[e]]'=[[e[x|->mu x:t.e]
]]$ by rule \ruleref{S\_Mu}.
		\item[Case $[[e]]=[[e1 e2]]$ and $[[e1]]$ is not a $\lambda$-term:] If
$[[e1]]=v$, there is no $[[e1']]$ such that $[[e1 --> e1']]$. Since $[[e1]]$ is
not a $\lambda$-term, there is no rule to reduce $[[e]]$. Thus there is no
$[[e]]'$ such that $[[e-->e']]$.
		
		Otherwise, there exists some $[[e1']]$ such that $[[e1 --> e1']]$. By the
induction hypothesis, $[[e1']]$ is unique reduction of $[[e1]]$. Thus by rule
\ruleref{S\_App}, $[[e]]'=[[e1' e2]]$ is the unique reduction for $[[e]]$.
		\item[Case $[[e]]=[[castdown e1]]$ and $[[e1]]$ is not a $[[castup]]$-term:]
If $[[e1]]=v$, there is no $[[e1']]$ such that $[[e1 --> e1']]$. Since $[[e1]]$
is not a $[[castup]]$-term, there is no rule to reduce $[[e]]$. Thus there is
no $[[e]]'$ such that $[[e-->e']]$.
		
		Otherwise, there exists some $[[e1']]$ such that $[[e1 --> e1']]$. By the
induction hypothesis, $[[e1']]$ is unique reduction of $[[e1]]$. Thus by rule
\ruleref{S\_CastDown}, $[[e]]'=[[castdown e1']]$ is the unique reduction for
$[[e]]$.
	\end{description}
\end{proof}

\begin{lem}[Uniqueness of $n$-step Reduction]\label{lem:appendix:uniquen}
	The $n$-step reduction $[[-->>]]$ is unique in the sense that given $[[e]]$
there is at most one $[[e']]$ such that $[[e]] [[-->>]] [[e']]$.
\end{lem}
...
star]]$.
		\item[Case $[[e=x]]$:] Trivial by rule \ruleref{T\_Var} and $[[t]]$ is the
unique type of $[[x]]$ if $[[x:t elt G]]$.
		\item[Case $[[e]]=[[e1 e2]]$:] By rule \ruleref{T\_App} and introduction
hypothesis, there exist unique expressions $[[t1]]$ and $[[t2]]$ such that $[[G
|- e1 : (Pi x:t1.t2)]]$, $[[G |- e2:t1]]$. Thus, from Lemma
\ref{lem:appendix:gen}, $[[t2[x |-> e2] ]]$ is the unique type of $[[e]]$.
		\item[Case $[[\x:t1.e1]]$:] By rule \ruleref{T\_Lam} and introduction
hypothesis, there exist unique expressions $[[t2]]$ such that $[[G |- (Pi
x:t1.t2):s]]$ and $[[G,x:t1 |- e:t2]]$. Thus, from Lemma
\ref{lem:appendix:gen}, $[[Pi x:t1.t2 ]]$ is the unique type of $[[e]]$.
		\item[Case $[[Pi x:t1.t2]]$:] By rule \ruleref{T\_Pi} and introduction
hypothesis, we have $[[G |- t1:s]]$ and $[[G, x:t1 |- t2:s]]$. Thus, from Lemma
\ref{lem:appendix:gen}, $[[s]]$ is the unique type of $[[e]]$.
		\item[Case $[[mu x:t.e1]]$:] By rule \ruleref{T\_Mu} and introduction
hypothesis, we have $[[G |- t:s]]$ and $[[G, x:t|-e:t]]$. Thus, from Lemma
\ref{lem:appendix:gen}, $[[t]]$ is the unique type of $[[e]]$.
		\item[Case $[[e]]=[[castup [t1] e1]]$:] From the premises of rule
\ruleref{T\_CastUp}, by induction hypothesis, we can derive the type of
$[[e1]]$ as $[[t2]]$, and check whether $[[t1]]$ is legal, i.e. its sorts is
$[[star]]$. If $[[t1]]$ is legal, by Lemma \ref{lem:appendix:unired}, there is
at most one $[[t1']]$ such that $[[t1 --> t1']]$. If such $[[t1']]$ does not
exist, then we report type checking fails. Otherwise, we examine if $[[t1']]$
is syntactically equal to $[[t2]]$, i.e. $[[t1' =a t2]]$. If the equality
holds, we obtain the unique type of $[[e]]$ which is $[[t1]]$. Otherwise, we
...
$[[e1]]$ as $[[t1]]$. By Lemma \ref{lem:appendix:unired}, there is at most one
$[[t2]]$ such that $[[t1 --> t2]]$. If such $[[t2]]$ exists and its sorts is
$[[star]]$, we have found the unique type of $[[e]]$ is $[[t2]]$. Otherwise, we
report $[[e]]$ fails to type check.
	\end{description}
\end{proof}
...
exist expressions $[[t1]]$ and $[[t2]]$ such that $[[|- e1:(Pi x:t1.t2)]]$ and
$[[|-e2:t1]]$. Consider whether $[[e1]]$ is a value:
    		\begin{itemize}
    		    \item If $[[e1]]=v$, by Lemma \ref{lem:appendix:gen}(3), it must be a
$\lambda$-term such that $[[e1 == \x:t1.e1']]$ for some $[[e1']]$ satisfying
$[[|- e1':t2]]$. Then by rule \ruleref{S\_Beta}, we have $[[(\x:t1.e1') e2 -->
e1' [x |-> e2] ]]$. Thus, there exists $[[e' == e1' [x |-> e2] ]]$ such that
$[[e --> e']]$.
    		    \item Otherwise, by induction hypothesis, there exists $[[e1']]$ such
...
there exist expressions $[[t1]]$ and $[[t2]]$ such that $[[|- e1:t1]]$ and
...
satisfying $[[|- e1':t2]]$. Then by rule \ruleref{S\_CastDownUp}, we can obtain
...
\ruleref{SC\_Case} and \ruleref{SC\_CaseMatch}.
    \begin{description}
...
(unfoldn e1') t <<e>>]]$. And by \ruleref{S\_CastDown}, we have the result
$[[(unfoldn e1) t <<e>> --> (unfoldn e1') t <<e>>]]$.
        \item[Case \scriptsize{$\ottdruleSCXXCaseMatch{}$}:] $\quad$ \\
        By rule \ruleref{TRdecl\_data}, $[[Ki == \ <<u:T>> . \<<x:T>> . foldn
[D@<<u>>] (\aa:star.\<<bb : <<T>> -> aa >> . bbi <<x>>)]]$. By
\ruleref{TRpat\_Alt} and \ruleref{TR\_Case}, we have $[[Gs |- case Ki <<u>>
...
a core language context $[[G]]$ such that $[[|- Gs ~> G]]$.
    \begin{description}
        \item[Case \ruleref{TR\_Ax}:] Trivial. $[[e]] = [[t]] = [[star]]$ and
$[[Gs |- star:star]]$ holds by rule \ruleref{T\_Ax}.
        \item[Case \ruleref{TR\_Var}:] Trivial. By rule \ruleref{T\_Var}, we
have $[[|- Gs ~> G]]$, then $[[x]]:[[t]] [[elt]] [[G]]$ where $[[Gs |-
A:star~>t]]$.
        \item[Case \ruleref{TR\_App}:] Suppose $[[Gs |- E1 E2 : A1[x |-> E2] ~>
...
rule \ruleref{T\_App}, we have $[[G |- e1 e2 : t1 [x |-> e2] ]]$.
        \item[Case \ruleref{TR\_Lam}:] Suppose $[[Gs |- (\x:A1.E):(Pi x:A1.A2)
...
rule \ruleref{T\_Lam}, we have $[[G |- (\x:t1.e):(Pi x:t1.t2)]]$.
        \item[Case \ruleref{TR\_Pi}:] Suppose $[[Gs |- (Pi x:A1.A2):r ~> Pi
x:t1.t2]]$. By induction hypothesis, we have $[[G |- t1 : star]]$ where $[[Gs
|- A1 : s ~> t1]]$. And $[[G, x : t1 |- t2 : star]]$ where $[[Gs, x: A1 |- A2 :
r ~> t2]]$. Thus by rule \ruleref{T\_Pi} $[[G |- (Pi x:t1.t2) : star]]$.
        \item[Case \ruleref{TR\_CastUp}:] Suppose $[[Gs |- (castup[A1] E):A1 ~>
...
\ruleref{T\_CastUp}, we have $[[G |- castup[t1] e : t1]]$.
...
\ref{lem:appendix:src:redtr}, we obtain $[[t1 --> t2]]$. Thus, by rule
\ruleref{T\_CastDown}, we have $[[G |- castdown e : t2]]$.
...
\ruleref{T\_Mu}, we have $[[G |- (mu x:t.e) : t]]$.
...
\ruleref{TRpat\_Alt}, we have
        ...
        \begin{align*}
            ...
            &[[Gs |- B1[<< u |-> U >>]:star ~> T ]]
        \end{align*}
        By rule \ruleref{TRdecl\_Data}, we have $[[D == mu X : (<<u:T*>>) ->
...
that by rule \ruleref{T\_Lam}, $[[G |- e2 : <<T>> -> t]]$. Therefore, by rule
\ruleref{T\_App}, we obtain $[[G |- (unfoldn e1) t <<e2>> : t]]$, which follows
the result.
    \end{description}
\end{proof}
