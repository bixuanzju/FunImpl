...
For syntax, we add the polymorphic recursive expression $\mu \, \ottmv{x}  \ottsym{:}  \tau  \ottsym{.}  \ottnt{e}$
...
have a higher kind like $\star  \rightarrow  \star$ to take parameters.
\bruno{what do you mean by polymorphic?  Perhaps a sentence is needed
  to clarify that}. \linus{Done.}  For operational semantics, we add
the rule \ruleref{S\_Mu} to define the unrolling operation of
recursion, which results in $\ottnt{e}  \ottsym{[}  \ottmv{x}  \mapsto  \mu \, \ottmv{x}  \ottsym{:}  \tau  \ottsym{.}  \ottnt{e}  \ottsym{]} \,$. For typing, we
...
recursion $\mu \, \ottmv{x}  \ottsym{:}  \tau  \ottsym{.}  \ottnt{e}$ should have the same type $\tau$ as the
binder $\ottmv{x}$ and also the inner expression $\ottnt{e}$.
...
%This is different from conventional term-level fixpoint that is
%usually treated as values \cite{zombie:popl14}
\bruno{I don't follow
  this? What is TAPL's treatement of recursion for example? I guess 
you are saying that in zombie recursion produces a value. But is this
the standard? or is zombie that is special?} \linus{Removed.}

Recall the factorial function example (Section
\ref{sec:overview:recursion})\bruno{use ``Section'' instead}, which
...
\begin{align*}
    ...
    - 1))
\end{align*}
By rule \ruleref{T\_Mu}, the type of $\mathsf{fact}$ is $\mathsf{Int}  \rightarrow  \mathsf{Int}$. Thus, we can apply $\mathsf{fact}$ to an integer, say $1$,
...
reduction. Also with standard reduction rules \ruleref{S\_Mu} and
...
\bruno{Mention that we assume
  and if construct, as well as standard reduction rules in the
  example.}\linus{Done.}
