...
For syntax, we add the polymorphic recursion operator $\mu \, \ottmv{x}  \ottsym{:}  \tau  \ottsym{.}  \ottnt{e}$ to
represent general recursion in both terms and types.
\bruno{what do you mean by polymorphic?  Perhaps a sentence is needed
  to clarify that}. 
For operational semantics, we add the rule
\ruleref{S\_Mu} to define the unrolling operation of recursion,
...
usually treated as values \cite{zombie:popl14}\bruno{I don't follow
...
the standard? or is zombie that is special?}.

Recall the factorial function example (\S
\ref{sec:overview:recursion})\bruno{use ``Section'' instead}, which can be defined as a $ \mu $-term
as follows:
\begin{align*}
...
we will get an integer as the result.  By rules \ruleref{S\_Mu} and
...
general recursion.
\bruno{Mention that we assume and if construct, as well as standard
reduction rules in the example.}
