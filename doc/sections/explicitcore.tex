...
For the decidability, we need to show there exists a type checking algorithm, which returns a unique type for a well-formed expression and never loops forever. For most expression typing rules, it is straightforward to follow the syntax-directed judgement to derive a decidable type checking procedure. For rules \ruleref{T\_CastUp} and \ruleref{T\_CastDown}, one of premises needs to judge if two types $\tau_{{\mathrm{1}}}$ and $\tau_{{\mathrm{2}}}$ holds follows the one-step reduction relation, i.e. check if $\tau_{{\mathrm{1}}}  \longrightarrow  \tau_{{\mathrm{2}}}$ holds.
...
\begin{lem}[Uniqueness of one-step reduction]\label{lem:core:unique}
...
With this result, checking if $\tau_{{\mathrm{1}}}  \longrightarrow  \tau_{{\mathrm{2}}}$ holds is decidable and rules \ruleref{T\_CastUp} and \ruleref{T\_CastDown} are therefore decidable. We can finally conclude the decidability of type checking:

\begin{lem}[Decidability of type checking]\label{lem:core:decide}
