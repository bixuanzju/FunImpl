...
We introduce two new primitives $ \kw{cast}^{\uparrow} $ and $ \kw{cast}_{\downarrow} $
...
$ \kw{cast}_{\downarrow} $ stands for the $\beta$-reduction of types, while
$ \kw{cast}^{\uparrow} $ is the inverse (Examples will be given in later typing sections).
...
application, and in the $ \kw{cast}_{\downarrow} $ inner expression
respectively. The reduction rules are \emph{weak} in the sense that it
is not allowed to reduce inside a $\lambda$-term or $ \kw{cast}^{\uparrow} $-term
...
$\kw{cast}^{\uparrow} \, \ottsym{[}  \tau_{{\mathrm{1}}}  \ottsym{]} \,  \ottnt{e}$ expands the type of $\ottnt{e}$ from $\tau_{{\mathrm{2}}}$ to
$\tau_{{\mathrm{1}}}$, while $\kw{cast}_{\downarrow} \, \ottnt{e}$ reduces the type of $\ottnt{e}$ from
$\tau_{{\mathrm{2}}}$ to $\tau_{{\mathrm{3}}}$. For example, assume $\Gamma  \vdash  \ottnt{e_{{\mathrm{1}}}}  \ottsym{:}  \kw{Int}$ and $\Gamma  \vdash  \ottnt{e_{{\mathrm{2}}}}  \ottsym{:}  \ottsym{(}  \lambda  \ottmv{x}  \ottsym{:}  \star  \ottsym{.}  \ottmv{x}  \ottsym{)} \, \kw{Int}$. Note that the following reduction holds:
\[ \ottsym{(}  \lambda  \ottmv{x}  \ottsym{:}  \star  \ottsym{.}  \ottmv{x}  \ottsym{)} \, \kw{Int}  \longrightarrow  \kw{Int}\]
Thus, we can obtain the following derivation of $\ottnt{e_{{\mathrm{1}}}}$ and $\ottnt{e_{{\mathrm{2}}}}$:
\[
\inferrule{\Gamma  \vdash  \ottnt{e_{{\mathrm{1}}}}  \ottsym{:}  \kw{Int} \\ \Gamma  \vdash  \ottsym{(}  \lambda  \ottmv{x}  \ottsym{:}  \star  \ottsym{.}  \ottmv{x}  \ottsym{)} \, \kw{Int}  \ottsym{:}  \star \\ \ottsym{(}  \lambda  \ottmv{x}  \ottsym{:}  \star  \ottsym{.}  \ottmv{x}  \ottsym{)} \, \kw{Int}  \longrightarrow  \kw{Int}}{\Gamma  \vdash  \ottsym{(}  \kw{cast}^{\uparrow} \, \ottsym{[}  \ottsym{(}  \lambda  \ottmv{x}  \ottsym{:}  \star  \ottsym{.}  \ottmv{x}  \ottsym{)} \, \kw{Int}  \ottsym{]} \,  \ottnt{e_{{\mathrm{1}}}}  \ottsym{)}  \ottsym{:}  \ottsym{(}  \lambda  \ottmv{x}  \ottsym{:}  \star  \ottsym{.}  \ottmv{x}  \ottsym{)} \, \kw{Int}}
\]
\[
\inferrule{\Gamma  \vdash  \ottnt{e_{{\mathrm{2}}}}  \ottsym{:}  \ottsym{(}  \lambda  \ottmv{x}  \ottsym{:}  \star  \ottsym{.}  \ottmv{x}  \ottsym{)} \, \kw{Int} \\ \Gamma  \vdash  \kw{Int}  \ottsym{:}  \star \\ \ottsym{(}  \lambda  \ottmv{x}  \ottsym{:}  \star  \ottsym{.}  \ottmv{x}  \ottsym{)} \, \kw{Int}  \longrightarrow  \kw{Int}}{\Gamma  \vdash  \ottsym{(}  \kw{cast}_{\downarrow} \, \ottnt{e_{{\mathrm{2}}}}  \ottsym{)}  \ottsym{:}  \kw{Int}}
\]
...
$\tau_{{\mathrm{1}}}  \longrightarrow  \tau_{{\mathrm{2}}}$ and $\tau_{{\mathrm{1}}}  \longrightarrow  \tau'_{{\mathrm{2}}}$. Then the type of $\kw{cast}_{\downarrow} \, \ottnt{e}$ can be
...
\begin{lem}[Decidability of Type Checking \ecore]\label{lem:ecore:decide}
...
Because of the uniqueness of one-step reduction, we can generalize one-step
...
\begin{flalign*}
    & \kw{cast}_{\uparrow}^n  [ \tau_{{\mathrm{1}}}, \dots, \tau_{\ottmv{n}} ] \ottnt{e} &  \triangleq ~  &  \kw{cast}^{\uparrow}  [ \tau_{{\mathrm{1}}} ]
( \kw{cast}^{\uparrow}  [ \tau_{{\mathrm{2}}} ] (\dots (  \kw{cast}^{\uparrow}  [ \tau_{\ottmv{n}} ] \ottnt{e} ) \dots )) \\
    & \kw{cast}_{\downarrow}^n  \ottnt{e} &  \triangleq ~  & \underbrace{ \kw{cast}_{\downarrow}  ( \kw{cast}_{\downarrow}  (\dots
(  \kw{cast}_{\downarrow} }_n \ottnt{e}) \dots ))
\end{flalign*}
...
them out in the $ \kw{cast}_{\uparrow}^n $ operator. Finally, we can have $n$-step \cast
